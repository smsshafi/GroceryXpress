...
<?php
    ...
    else
    {
        ...
        if (isset($_GET['category']) && $_GET['category'] != "") {
            $num_pages = GetNumRecipeSearchPages($_GET['category'], true);
            $result = SearchRecipeCategory($_GET['category'], $page);
        }
		else {
            $num_pages = GetNumRecipeSearchPages($_GET['search']);
			$search = $_GET['search'];
        
            $result = SearchRecipe($_GET['search'], $page);
        }
        
    }

?>
<?php $page_title = 'Find Recipe'; ?>
<?php echo $sDocType; ?>
	...
	<div class="content"> 
        <div class="fullcolumn">
				...
				<div style="padding: 10px;">
					<form id="frm_search" name="frm_search" method="GET" action="findrecipe.php" style="position: relative;">
						<label for="search"></label>
						 <input type="text" name="search" 
						id="search" style="width: 400px;" value="<?php echo $search; ?>"/> 
						<input type="submit" id="do_search" value="Find"/> 
                        <br/><i>Example: Skillet Salmon</i> 
		...
		<div class="fullcolumn">
            ...
            if (!mysql_num_rows($result) == 0) {     
                $search_pages =	'<div style="margin: 10px; border: 1px dashed; padding-left: 10px;">Page: &nbsp;';
